...
public class WeaponController : MonoBehaviour
{
    ...
    public void PauseMenuWeaponControl()
    {
        EquipUnequip();
        SwitchWeapon();
    }
    ...
    private void EquipUnequip()
    {
        ...
        {
            if (Input.GetKeyDown(KeyCode.Z) || Input.GetMouseButtonDown(0) || Input.GetMouseButtonDown(1))
            {
                ReadyWeapon();
            }
        }
    ...
    {
        if (Time.time >= NextSwap && weaponEquipped && Input.GetKeyDown(KeyCode.Q))
        {
    ...
    public void Reload(bool force = false)
    {
        if(currentWeaponGameObject && currentWeaponGameObject.GetComponent<Weapon>().ReturnWeaponType() == WeaponType.Ranged)
        {
            if (force)
            {
                currentWeaponGameObject.GetComponent<Ranged>().Reload(force);
            }
            else if (Input.GetKeyDown("r"))
            {
                currentWeaponGameObject.GetComponent<Ranged>().Reload();
            } 
        }
    }
