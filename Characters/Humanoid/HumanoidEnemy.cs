...
public class HumanoidEnemy : EnemyTemplateMaster
{
    ...
    public void ExternalMovement(Vector3 loc_in, float speed = 3f)
    {
        agent.speed = speed * SpeedMult();
        agent.SetDestination(loc_in);
    }
    ...
    {
        HumanoidLogicMode humanoidLogicMode = EWC.Attack();
        ...
        else if (humanoidLogicMode == HumanoidLogicMode.Kiting)
        {
            Kite(); //Var agent.speed
        }
        else if (humanoidLogicMode == HumanoidLogicMode.Kiting_reposition)
        {
    ...
    public override void EnableAI(bool RallyAllies)
    {
    ...
    private int kite_sweet_spot = 3; //Either side
    private bool runToOutside = false;
    private bool runToInside = false;
    protected void Kite()
    {
        faceTarget = true;
        int kite_radius = 0;
        ...
        if(runToOutside && current_distance > kite_radius) //This will cause to run to center
        {
            runToOutside = false;
        }
        else if(runToInside && current_distance < kite_radius) //This will cause to run to center
        {
            runToInside = false;
        }


        if (runToOutside || current_distance < (kite_radius - kite_sweet_spot)) //Too close
        {
            runToOutside = true;
            agent.speed = baseKiteBackSpeed * SpeedMult();
            agent.SetDestination(transform.position - vec_diff); //Run away
        }
        else if (runToInside || current_distance > (kite_radius + kite_sweet_spot)) //Too far
        {
            runToInside = true;
            agent.speed = baseKiteForwardSpeed * SpeedMult();
            agent.SetDestination(Return_Current_Target().position); //Run towards
            Roll();
        }
        else
        {
    ...
    protected void Charge()
    {
        ...
        agent.SetDestination(Return_Current_Target().position);
    }
